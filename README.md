# <div align="center">欢迎来到 `im10furry · 黑啤` 的小宇宙</div>

<div align="center">
  <img src="./assets/header.svg" width="100%" alt="im10furry GitHub Profile Header" />
</div>

<div align="center">

  ![Profile Views](https://komarev.com/ghpvc/?username=im10furry&label=PROFILE%20VIEWS&color=ff9fc9&style=for-the-badge)
  ![ACG Mode](https://img.shields.io/badge/ACG%20MODE-ON-ffc8dd?style=for-the-badge)
  ![Open Source](https://img.shields.io/badge/OPEN%20SOURCE-LOADING-bde0fe?style=for-the-badge)

</div>

<div align="center">

> 「把灵感写进代码，把喜欢的东西做成作品。」

</div>

<div align="center">
  <sub>GitHub 上是 <code>im10furry</code>，中文别称是 <code>黑啤</code>。</sub>
</div>

## 🌸 关于我

| 属性 | 内容 |
| --- | --- |
| `ID` | `im10furry` |
| `中文别称` | `黑啤` |
| `职业设定` | 二次元浓度偏高的开发者 |
| `当前状态` | 一边写代码，一边给自己的仓库加灵魂 |
| `关注方向` | `Web` / `Automation` / `Creative Coding` / `Open Source` |
| `日常爱好` | 动漫、游戏、折腾页面、做有趣的小工具 |
| `目标` | 让作品看起来不只是能跑，而是真的有记忆点 |

## ✨ 当前任务栏

- 🔭 正在折腾：`个人项目 / GitHub 美化 / 好玩的前端效果`
- 🌱 正在学习：`更优雅的工程化、交互表达、自动化能力`
- 👯 想一起合作：`开源项目、创意页面、小工具、ACG 向内容`
- 💬 可以来聊：`前端、脚本、GitHub 主页、二次元文化`
- ⚡ 特殊设定：`看到好看的 UI 会忍不住想自己做一版`

## 🪄 技能树

<div align="center">
  <img src="https://img.shields.io/badge/JavaScript-f6d365?style=flat-square&logo=javascript&logoColor=3a2e2e" alt="JavaScript" />
  <img src="https://img.shields.io/badge/TypeScript-a8dadc?style=flat-square&logo=typescript&logoColor=1d3557" alt="TypeScript" />
  <img src="https://img.shields.io/badge/Python-cdb4db?style=flat-square&logo=python&logoColor=2b2d42" alt="Python" />
  <img src="https://img.shields.io/badge/Node.js-b7e4c7?style=flat-square&logo=node.js&logoColor=1b4332" alt="Node.js" />
  <img src="https://img.shields.io/badge/React-90e0ef?style=flat-square&logo=react&logoColor=023e8a" alt="React" />
  <img src="https://img.shields.io/badge/Vue-ade8f4?style=flat-square&logo=vuedotjs&logoColor=1b4332" alt="Vue" />
  <img src="https://img.shields.io/badge/HTML5-ffd6a5?style=flat-square&logo=html5&logoColor=9c6644" alt="HTML5" />
  <img src="https://img.shields.io/badge/CSS3-ffcad4?style=flat-square&logo=css&logoColor=6d597a" alt="CSS3" />
  <img src="https://img.shields.io/badge/Git-e9edc9?style=flat-square&logo=git&logoColor=bc6c25" alt="Git" />
  <img src="https://img.shields.io/badge/GitHub-d8e2dc?style=flat-square&logo=github&logoColor=3d405b" alt="GitHub" />
</div>

## 📊 角色面板

<div align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=im10furry&show_icons=true&theme=rose_pine&hide_border=true&bg_color=00000000&title_color=eb6f92&icon_color=9ccfd8&text_color=575279" alt="GitHub Stats" />
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=im10furry&layout=compact&theme=rose_pine&hide_border=true&bg_color=00000000&title_color=eb6f92&text_color=575279" alt="Top Languages" />
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=im10furry&bg_color=fffafc&color=6d597a&line=eb6f92&point=9ccfd8&area=true&hide_border=true" width="95%" alt="Contribution Graph" />
</div>

## 🎐 联系方式

| 平台 | 账号 |
| --- | --- |
| `GitHub` | [@im10furry](https://github.com/im10furry) |
| `WeChat` | `Liuwj315` |
| `QQ` | `1936409761` |
| `Email` | `xiaoliuwj.china@gmail.com` |

> 如果你是因为项目、页面设计、脚本工具或者单纯同好交流而来，直接联系我就行。

## 🍡 Fun Fact

- 我很在意一个仓库的第一印象，所以 README 通常不会随便写。
- 对我来说，代码是功能，风格是角色立绘，两者都不能缺席。

---

<div align="center">
  <sub>Made with love, caffeine, and a little bit of ACG magic.</sub>
</div>
